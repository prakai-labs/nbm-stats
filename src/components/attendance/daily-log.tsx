...
export function DailyLog({ date, teacherName }: DailyLogProps) {
  ...
  useEffect(() => {
    if (data !== undefined && date !== lastLoadedDate) {
      if (log) {
  ...
  }, [data, date, log, teacherName, lastLoadedDate])
      ...
      <CardContent className="space-y-3 p-4">
        <div className="space-y-1">
          ...
          <Textarea
            ...
            className="min-h-[120px] disabled:opacity-80 disabled:cursor-default"
            disabled={!isEditing}
          />
        </div>
        <div className="flex items-end gap-2">
          <div className="flex-1 space-y-1">
            ...
            <Input
              ...
              className="h-9 disabled:opacity-80 disabled:cursor-default"
              disabled={!isEditing}
            />
          </div>
          {log && !saveMutation.isPending && !isEditing && (
